...
#ifdef PLAT_UNIX
#ifdef USE_SDL2
int SDLtoFS2[SDL_NUM_SCANCODES];	// SDL2: indexed by scancode, not keycode
#else
...
#if defined(PLAT_UNIX) && defined(USE_SDL2)
// map SDL2 scancodes -> FreeSpace key codes
void FillSDLArray ()
{
	...
	SDLtoFS2[SDL_SCANCODE_SLASH] = KEY_DIVIDE;
...
int key_numlock_is_on()
{
	...
	if ( keys && keys[SDL_SCANCODE_NUMLOCKCLEAR] ) {
		return 1;
	}
#elif defined(PLAT_UNIX)
	int keys[SDLK_LAST];
